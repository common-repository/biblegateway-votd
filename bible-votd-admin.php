...
if ( !class_exists( 'dz_biblegateway_votd_admin' ) ) {
	...
	class dz_biblegateway_votd_admin {
		 ...
		 */
		const bg_api_uri = 'https://www.biblegateway.com/votd/get/?format=json&version=%1$s';
...
<?php screen_icon(); ?>
	...
	<p>The BibleGateway.com plugin is independently written and maintained by <a href="https://dave.zaikos.com/">Dave Zaikos</a> and in no way affiliated with BibleGateway.com.</p>
...
<?php
		...
		private function remote_get_json_helper( $json ) {
			...
			if ( !empty( $filtered['audiolink'] ) ) {
				...
				$verse .= '<img width="13" height="12" src="https://www.biblegateway.com/resources/audio/images/sound.gif" alt="Listen to chapter" /></a>';
			}

			$verse .= ' <a href="' . esc_url( $filtered['copyrightlink'], array( 'http', 'https' ) ) . '">';
			$verse .= esc_html( rtrim( $filtered['copyright'], '.' ) ) . '</a>. ';
			$verse .= 'Powered by <a href="https://www.biblegateway.com/">BibleGateway.com</a>.';
